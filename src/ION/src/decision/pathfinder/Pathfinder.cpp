#include <ION/decision/move_straight_line/Pathfinder.hpp>
#include <ros/ros.h>
	...
	namespace decision{
		namespace pathfinder{
			...
			Grid::Grid(
				...
				geometry_msgs::Pose2D::ConstPtr& start, 
				geometry_msgs::Pose2D::ConstPtr& goal)
			{
				...
				start = getTileAt(start->x, start->y);
				goal = getTileAt(goal->x, goal->y);
			}


			std::vector<Tile*> computeShortestPath(, bool reduce=false){
